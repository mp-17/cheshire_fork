...
int main(void) {
    ...
    //////////////////////////////////////////////////////////////////
    // TEST: test vload and vstore for all the vl values
    //////////////////////////////////////////////////////////////////

    // Loop through different avl, from 0 to avlmax
    for (uint64_t avl = 0; avl <= VLMAX; avl++) {
      // Reset vl, vstart, reset exceptions
      RVV_TEST_INIT(vl, avl);

      // Check vload and vstore for this value of vl
      STUB_EX_OFF;
      ...
      for (uint64_t i = 0; i < vl; i++) {
        address_load[i]  = vl + i + MAGIC_NUM;
      }

      // Load and store the values back
      asm volatile("vle64.v	v0, (%0)" : "+&r"(address_load));
      asm volatile("vse64.v v0, (%0)" : "+&r"(address_store));

      // Check that the stored values are the same as the loaded ones
      for (uint64_t i = 0; i < vl; i++) {
    ...
    for (uint64_t avl = 0; avl <= VLMAX; avl++) {
      ...
      for (uint64_t vstart_val = 0; vstart_val <= vl + 1; vstart_val++) {
        // Reset vl, vstart, reset exceptions.
        RVV_TEST_INIT(vl, avl);
